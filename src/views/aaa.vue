                  ...
                  >
                    ...
                    <template v-else>
                      <div class="file-icon-wrapper">
                        ...
                        <span class="item-duration-tag" v-if="item.duration !== '未知'">{{ item.duration }}</span>
                      </div>
                      <span class="item-name">{{ item.name }}</span>
                    </template>
                  </div>
              ...
              <div v-if="resultImages.length > 0" class="browser-layout">
                <div class="image-main">
                  ...
                  <div class="img-info-overlay">
                    关键帧：{{ resultImages.indexOf(currentImgUrl) + 1 }} / {{ resultImages.length }}
                  </div>
                </div>
                <!-- 侧边缩略图网格 -->
          ...
          <div class="log-section">
            ...
            <div class="log-box">
              <div v-for="(l, i) in logs" :key="i" class="log-item">
                ...
                <span class="l-msg">
                  <template v-if="l.tag === '识别'">
                    视频时间: {{ l.video_time }}, 帧序号: {{ l.frame }},
                  </template>
                  {{ l.message }}
                </span>
              </div>
...
const currentList = computed(() => {
  if (pathStack.value.length === 0) return serverVideosTree.value;
  return pathStack.value[pathStack.value.length - 1].children || [];
});
...
.explorer-main { padding: 15px; height: 260px; overflow-y: auto; }

.folder-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
.folder-item { 
  display: flex; flex-direction: column; align-items: center; 
  cursor: pointer; padding: 10px; border-radius: 6px; border: 1px solid transparent; transition: 0.2s;
}
.folder-item:hover { background: #ecf5ff; }
.is-active-file { background: #f0f9eb !important; border: 1px solid #67c23a; }

.folder-icon { font-size: 40px; color: #e6a23c; }
.file-icon { font-size: 40px; color: #409EFF; }
.file-icon-wrapper { position: relative; display: flex; align-items: center; justify-content: center; }
.item-duration-tag { position: absolute; bottom: 2px; right: -5px; background: rgba(0,0,0,0.6); color: #fff; font-size: 9px; padding: 0 4px; border-radius: 2px; }
.item-name { margin-top: 5px; font-size: 11px; text-align: center; width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
