      ...
      <el-col :span="9">
        <el-card class="side-card">
          ...
          <el-form label-position="top">
              ...
              <div class="explorer-main">
                ...
                <div class="explorer-list">
                  <div 
                    ...
                    class="list-row"
                    :class="{ 'is-active-file': selectedLibraryPaths.includes(item.url) }"
                  ...
                  >
                    ...
                    <div class="item-meta">
                      <el-checkbox 
                        ...
                        :model-value="selectedLibraryPaths.includes(item.url)" 
                        @click.stop 
                      />
            ...
            <!-- 2. 事件多选 -->
            <el-form-item label="监控异常事件 (支持多选)">
            ...
            <el-form-item>
              <template #label>
                <div class="label-tip">
                  自定义描述 (自然语言扩展)
                  <el-tooltip content="描述您想要识别的特定交通行为，将调用大模型进行分析。" placement="top">
              ...
              <el-input
                ...
                type="textarea"
                :rows="3"
                placeholder="例如：识别一辆在路口徘徊的白色面包车"
              />
            </el-form-item>
        ...
        <el-card class="main-card">
          <template #header>
            <div class="flex-between">
              <span>巡检图像预览 (已捕获: {{ resultImages.length }} 张)</span>
              <div class="header-right">
                <span v-if="analyzing" class="prog-text">处理进度: {{ progress }}%</span>
                <el-tag :type="analyzing ? 'danger' : 'success'">{{ analyzing ? '正在分析' : '空闲' }}</el-tag>
              </div>
            </div>
          </template>
          ...
          <div class="result-viewer">
            ...
            <div class="main-image-box">
              ...
              >
                <template #placeholder><div class="img-loading">加载中...</div></template>
              </el-image>
          ...
          <div class="log-section">
            ...
            <div class="log-box">
              ...
              <div v-if="logs.length === 0" class="log-empty">等待图像任务启动...</div>
            </div>
          </div>
        </el-card>
      </el-col>
...
import { 
  UploadFilled, Setting, InfoFilled, Picture, 
  FolderOpened, HomeFilled, Monitor 
...
const selectedLibraryPaths = ref([]); 
...
onMounted(async () => {
  try {
    ...
    serverImagesTree.value = res.data;
  } catch (e) {
    ElMessage.error("后端素材库连接失败");
  }
});
...
const toggleLibrarySelect = (file) => {
  const index = selectedLibraryPaths.value.indexOf(file.url);
  if (index > -1) {
    selectedLibraryPaths.value.splice(index, 1);
  } else {
    selectedLibraryPaths.value.push(file.url);
    // 预览最新选中的图
...
const doStart = async () => {
  ...
  const hasServer = sourceType.value === 'server' && selectedLibraryPaths.value.length > 0;
  if (!hasLocal && !hasServer) return ElMessage.warning("请准备素材");

  analyzing.value = true;
  try {
    const res = await startImageAnalysis({
      files: sourceType.value === 'local' ? localFileList.value.map(f => f.raw) : null,
      paths: sourceType.value === 'server' ? selectedLibraryPaths.value : null,
      events: events.value,
...
.explorer-main { padding: 0; height: 260px; overflow-y: auto; }
...
.item-meta { flex: 1; text-align: right; }

.icon-folder { color: #e6a23c; font-size: 18px; }
.icon-image { color: #67c23a; font-size: 18px; }
.folder-text { color: #c0c4cc; font-style: italic; font-size: 12px; }

/* --- 展示与结果预览 --- */
.result-viewer { display: flex; height: 420px; background: #000; border-radius: 8px; overflow: hidden; }
...
.log-box { height: 160px; overflow-y: auto; background: #1e1e1e; padding: 12px; border-radius: 6px; border: 1px solid #333; }
.log-item { font-family: monospace; font-size: 13px; color: #dcdcdc; margin-bottom: 6px; }
...
.label-tip { display: flex; align-items: center; gap: 5px; }
.info-icon { color: #409EFF; cursor: help; }
.v-empty { color: #666; text-align: center; }
</style>